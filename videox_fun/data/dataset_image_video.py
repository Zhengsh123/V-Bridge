...
class ImageVideoDataset(Dataset):
    def __init__(
        ...
        enable_only_crop=False, 
        video_crop_size=None, 
        enable_video_resize=False, 
    ):
        ...
        self.enable_only_crop = enable_only_crop
        self.enable_video_resize = enable_video_resize
        ...
        self.video_crop_size = tuple(video_crop_size) if (video_crop_size is not None and not isinstance(video_crop_size, int)) else (video_crop_size, video_crop_size)
        ...
        self.video_transforms_only_crop = transforms.Compose(
            [
                transforms.RandomCrop(self.video_crop_size),
                transforms.Normalize(mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5], inplace=True),
            ]
        )
    ...
    def get_batch(self, idx):
        ...
        if data_info.get('type', 'image')=='video':
            ...
            with VideoReader_contextmanager(video_dir, num_threads=2) as video_reader:
                ...
                if not self.enable_bucket:
                    if self.enable_only_crop:
                        pixel_values = self.video_transforms_only_crop(pixel_values)
                    else:
