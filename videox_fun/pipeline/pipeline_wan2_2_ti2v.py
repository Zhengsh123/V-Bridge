...
class Wan2_2TI2VPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if init_video is not None:
                    # Original code (single batch only):
        ...
        # Timing measurements after denoising loop
        timing_results = {}
        ...
        timing_results['decode_latents'] = decode_start.elapsed_time(decode_end)

        # Step 2: Video post-processing (if applicable)
        if output_type != "latent" and output_type != "numpy":
            ...
            torch.cuda.synchronize()
            timing_results['postprocess_video'] = postprocess_start.elapsed_time(postprocess_end)

        # Step 3: Offload all models
        offload_start = torch.cuda.Event(enable_timing=True)
        offload_end = torch.cuda.Event(enable_timing=True)
        offload_start.record()

        self.maybe_free_model_hooks()

        offload_end.record()
        torch.cuda.synchronize()
        timing_results['maybe_free_model_hooks'] = offload_start.elapsed_time(offload_end)

        # Step 4: Tensor conversion (if applicable)
        if not return_dict:
            ...
            convert_end.record()
            torch.cuda.synchronize()
            timing_results['tensor_conversion'] = convert_start.elapsed_time(convert_end)

        # Print timing results
        print("\n=== Post-Denoising Timing Results ===")
        for step_name, elapsed_time in timing_results.items():
            print(f"{step_name}: {elapsed_time:.2f} ms")
        print(f"Total post-denoising time: {sum(timing_results.values()):.2f} ms")
        print("=" * 40)
